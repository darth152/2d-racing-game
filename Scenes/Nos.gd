...
func _process(_delta):
	#makes the nos pickup constantly move
	...
	#makes the nos pickup move faster with the track when accelerating
	...
	#makes the nos pickup move faster with the track when boosting
	if Obstacles.boosting == true:
		position.y += 12
		
	#makes the nos pickup move slower with the track when decelerating
	if Obstacles.braking == true:
		position.y += -3


#makes the nos pickup delete when not seen
func _on_VisibilityNotifier2D_screen_exited():
	queue_free()

#gives you +1 boost when picked up
